...
select party ,GDH_COMMDATE, expirydate,SUM(SumInsured),SUM(GrossPrem),SUM(NetPrem),SUM(Facult) from (
...
d.PDP_DESC AS Dept_Desc ,
--h.GDH_DOC_REFERENCE_NO AS PolicyNo,
h.GDH_COMMDATE,
...
and COALESCE(maxe.expiry,h.GDH_COMMDATE) BETWEEN '01-Sep-2018' AND '30-Sep-2018'
...
ORDER BY  h.pdt_doctype,h.gdh_documentno
 ...
 GROUP BY party ,GDH_COMMDATE, expirydate
